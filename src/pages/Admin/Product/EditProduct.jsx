  ...
  useEffect(() => {
    ...
    if (updateIsError) {
      Swal.fire(
        ...
        updateError?.error
          ? updateError?.error
          : "Product update Fail, please try again",
        "error"
      );
    }
  }, [isSuccess, updateIsError, updateError]);

  let content = null;
  if (isLoading) {
    return (content = <Spinner />);
  }
  if ((!isLoading && isError) || updateIsError) {
                ...
                <div className="border rounded p-4 mt-5">
                  <div className="relative overflow-x-auto">
                    <table className="border_table">
                      ...
                      <tbody>
                        {product?.varients?.map((color, colorIndex) => (
                          <tr key={colorIndex}>
                            <td>{color.color}</td>
                            <td>
                              {color.info?.map((size, sizeIndex) => (
                                <input
                                  key={sizeIndex}
                                  type="text"
                                  disabled
                            ...
                            <td>
                              {color.info?.map((sz, sizeIndex) => (
                                <input
                                  key={sizeIndex}
                                  type="number"
                                  defaultValue={sz.quantity}
                                  onChange={(e) =>
                                    handleInputChange(
                                      colorIndex,
                                      sizeIndex,
